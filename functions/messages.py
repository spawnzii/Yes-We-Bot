...
def send_infos(user):
    if get_user_infos(user) is False:
        ...
        embed.add_field(name="User not found", value=msg, inline=True)
...
def send_add_user(user):
    ...
    if add == "User already on the db":
        msg = f"User **{user}** is already on the database."
    if add == "User add":
        msg = f"User **{user}** add to the database."
...
def send_bugs():
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        checksum_futures = [executor.submit(checksum_feed, feed) for feed in feeds]
        old_hash_futures = [executor.submit(db_get_old_hash, hunter) for hunter in hunters]
